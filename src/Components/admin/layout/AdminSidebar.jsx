...
export function AdminSidebar() {
  const navigate = useNavigate(); // ✅ Sekarang ini akan jalan karena sudah diimport
  ...
  return (
    <div className="w-64 bg-slate-900 text-white min-h-screen flex flex-col fixed left-0 top-0 bottom-0 shadow-xl z-40">
        ...
        {menuItems.map((item) => {
          ...
          return (
            <NavLink
              key={item.href}
              to={item.href}
              // ✅ Menggunakan className function ({ isActive }) dari NavLink
              className={({ isActive }) =>
      ...
      {/* ========================================
       🔔 USER PROFILE SECTION (Bawah Sidebar)
       ======================================== */}
